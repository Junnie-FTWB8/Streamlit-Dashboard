...
if ngrams == "Unigram":
    with col1:
        ...
        st.bar_chart(pd.DataFrame({'Top Features': top_features, 'Top Coefficients': top_coeffs}).set_index('Top Features'))

    with col2:
        # Create a bar chart for bottom coefficients
        st.bar_chart(pd.DataFrame({'Bottom Features': bottom_features, 'Bottom Coefficients': bottom_coeffs}).set_index('Bottom Features'))
